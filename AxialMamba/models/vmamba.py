...
class VSSBlock(nn.Module):
    ...
    def _forward(self, input: torch.Tensor):
        x = input
        print("在vssblock之前的大小为：", x.shape)
        if self.ssm_branch:
            if self.post_norm:
                x = x + self.drop_path(self.norm(self.op(x)))
                print("在vssblock之后的大小为：", x.shape)
            else:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.classifier = nn.Sequential(OrderedDict(
            ...
            heda=nn.Conv2d(in_channels=self.num_features, out_channels=self.num_classes, kernel_size=1),
        ))
...
#     model = VSSM(
#         depths=[2, 2, 4, 2], dims=96, drop_path_rate=0.2,
#         patch_size=4, in_chans=3, num_classes=21,
#         ssm_d_state=64, ssm_ratio=1.0, ssm_dt_rank="auto", ssm_act_layer="gelu",
#         ssm_conv=3, ssm_conv_bias=False, ssm_drop_rate=0.0,
#         ssm_init="v2", forward_type="m0_noz",
#         mlp_ratio=4.0, mlp_act_layer="gelu", mlp_drop_rate=0.0, gmlp=False,
...
#         use_checkpoint=False, posembed=False, imgsize=224,
#     ).to('cuda:0')
#     x = torch.rand((16, 3, 224, 224)).to('cuda:0')
#     out = model(x)
#     print(out.shape)
...
if __name__ == '__main__':
    ...
    print("原始图片的大小：", input_image.shape)
    ...
    print(f'Non-trainable number of parameters: {non_trainable_params}')
